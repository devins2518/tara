...
fn genInlineBlock(self: *UTirGen, env: *Environment, node_idx: Ast.Node.Idx) UTirGenError!Inst.Ref {
    ...
    const return_value = switch (tags[@intFromEnum(node_idx)]) {
        .@"or" => try self.genBinOp(&block_env, node_idx, .@"or"),
        .@"and" => try self.genBinOp(&block_env, node_idx, .@"and"),
        .lt => try self.genBinOp(&block_env, node_idx, .lt),
        .gt => try self.genBinOp(&block_env, node_idx, .gt),
        .lte => try self.genBinOp(&block_env, node_idx, .lte),
        .gte => try self.genBinOp(&block_env, node_idx, .gte),
        .eq => try self.genBinOp(&block_env, node_idx, .eq),
        .neq => try self.genBinOp(&block_env, node_idx, .neq),
        .bit_and => try self.genBinOp(&block_env, node_idx, .bit_and),
        .bit_or => try self.genBinOp(&block_env, node_idx, .bit_or),
        .bit_xor => try self.genBinOp(&block_env, node_idx, .bit_xor),
        .add => try self.genBinOp(&block_env, node_idx, .add),
        .sub => try self.genBinOp(&block_env, node_idx, .sub),
